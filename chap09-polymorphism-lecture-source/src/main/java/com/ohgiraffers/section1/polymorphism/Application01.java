...
public class Application01 {
    ...
    * 다형성
    *
    * 다형성이란 하나의 인스턴스가 여러가지 타입을 가질 수 잇는 것을 의미한다.
    ...
    * 2. 상속을 기반으로 한 기술이기 때문에 상속관계에 있는 모든 객체는 동일한 메세지를
    *    수신할 수 있다. 동일한 메세지를 수신하고 처리할 때 객체별로 다르게 할 수 있다는 장정을 가짐.
    * 3. 확장성이 좋은 코드를 적성할 수 있다.
...
//        ((Tiger)a1).bite(); // 에러남
        ...
        System.out.println("a1이 Tiger인지 확인: " + (a1 instanceof Tiger));    // false
        System.out.println("a1이 Rabbit인지 확인: " + (a1 instanceof Rabbit));  // true
        // 상속받은 타입도 함께 가지고 있다.(다형성)
        System.out.println("a1이 Animal인지 확인: " + (a1 instanceof Animal)); // t
        System.out.println("a1이 Object인지 확인: " + (a1 instanceof Object)); //t
        ...
        if ((a1 instanceof Tiger)){
            ((Tiger)a1).bite();   /// 인식 x a1이 토끼라서 동작 안함
        }
