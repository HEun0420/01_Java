package com.ohgiraffers.section6;
...
public class finalkeyword {
    ...
    *
    * final이 사용되는 곳 -> 변경 불가의 읨
    * 1. 지역변수 : 초기화 이후에 값 변경 불가
    * 2. 매개변수 : 메소드 호출 시 전달한 인자 변경 불가
    * 3. 전역변수 : 인스턴스 생성 후, 초기화 이후 값 변경불가
...
//    }
    ...
    * final 키워드 사용시 초기화를 하지 않으면 에러가 발생한다.
     */
    // private staitc final int STATIC_NUM;

    /*
    * 생성자를 이용한 초기화는 불가능
    * 생성자는 인스턴스가 생서되는 시점에 호출되기 때문에 그 전에는 초기화가 일어나지 못한다.
    * 하지만 static은 프로그램이 start 될 때 할당되기 때문에
...
//
//    public FinalFildTest(double staticDouble){
//        STATIC_Double = stacticdouble;
//    }
//
