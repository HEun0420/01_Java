...
public class Application01 {
    public static void main(String[] args) {
         ...
         * Operator : Function과 똑같이 applyXXX() 메소드를 가지고 있는데, 매개변수로 연산을 한 후 통일된 타입
         * Predicate : 매개변수와 boolean 값을 반환하는 testXXX() 메소드를 가지고 있다. (매개변수를 활용해서 Boolean 값 리턴)
         */
        ...
        BiConsumer<String,LocalTime> biConsumer = (str, time) -> System.out.println(str + "이(가)출력됨");
        biConsumer.accept("hello world", LocalTime.now());

        /* InConsumer #accept(int value) : void : int값을 받아서 소비한다.*/
        ...
        ObjIntConsumer<java.util.Random> objIntConsumer =
                (((random, value) ->
                        System.out.println("0부터 " + value + " 전까지의 난수 발생" + random.nextInt())));
        objIntConsumer.accept(new java.util.Random(), 10);

        /* ObjLongConsumer<T>accept(T t, int value) : void : 객체 T와 long값을 받아 소비한다.*/
