...
public class Application {
    public static void main(String[] args) {
        ...
        * 믈래스 또한 부모 클래스와 자식클래스로 역할을 나누어서 부모가 가지는 멤버를 자식이
        * 물려받아 자기의 멤버인 것처럼 사용할 수 있도록 만든 기술
        *
        * 단순히 물려받는 개념보다, 자베에서의 상속은 부모클래스의 확장(extend)의 개념을 가진다.
        ...
        * 자식클래스가 저으이한 메소드대로 동작하도록 구현 몸체 부분을 새롭게 다시 작성하는 기술
        ...
        *          부모 클래스만 수정해도 전체적으로 적용된다. (유지 보수성 증가)
        *   1-2. 클래스 간의 계층 관계가 형성되며 다형성의 문법적인 토대가 된다.
        * 2. 단점
        *   1-1. 부모클래스의 기능을 추가/변경할 시 자식 클래스가 정상적으로 도앚갛는 지에 대한 예측이 힘들다.
        *   1-2. 부모클래스의 변경이 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우
        *        부모클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다.
        *   1-3. 부모클래스에서 의미있던 기능이, 자식클래스에서는 무의미할 수 있다.
         */
        ...
        *
        * fireCar is a car / car is a fireCar / 소방차는 차지만, 모든 소방차가 차인건 아니다.
         */

        /*
        * 객체 지향 설계 관점에서 바라보는 상속
        * 모든 객체는 자신이 수신한 메세지에 대한 응답을 해야하는 책임을 가지며, 그 책임의 규모는
        * 적절 해야한다. 적절한 책임을 가진 객체들이 서로 협력(메세지 수진과 응답)을 통해
        * 프로그램을 동작하는 것 => 객체지향 프로그램
         */
...
//        fireCar.isRunning(); // private 메소드에 접근 불가 -> protected 로 변경 후 사용 가능
