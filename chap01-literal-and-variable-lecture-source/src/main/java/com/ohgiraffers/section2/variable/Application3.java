...
public class Application3 {
    public static void main(String[] args) {
        ...
        *   2-2. 변수명이 합성어로 이루어진 경우, 첫단어는 소문자, 두번째 시작단어부터는 대문자로 시작한다. (comel-case)
        *   2-3. 단어와 단어 사이의 연결은 언더스코어(_)로 하지 않는다. (2-2 지켜라)
        *       상수를 사용할 때 특수하게 사용한다.
        *   2-4. 한글로 변수명을  짓는게 가능하지만, 권장되진 않는다. (한글을 취급하는 다양한 방식때문에 에러 유발)
        *   2-5. 변수 안에 저장된 값이 어떤 의미를 가지는지 명확하게 표현하도록한다.
        *   2-6. 전형적인 변수 이름이 있다면 가급적 사용하도록 한다.
        *   2-7. 명사형으로 작성 할 수 있도록 한다.
        *   2-8. boolean 형은 의문문으로 가급적 긍정의 형태로 네이밍을 한다.
         */
        ...
        //int a@ge;
        int _a_ge = 20; //언더스코어(_) 사용가능하다. 중간 마지막 상관 없음
        int $harp = 20; // $ 사용 가능하다. 처음 중간 마지막 상관 없음.

        // 암묵적 약속

        // 변수의 길이에는 제한이 없지만, 적당히 하자.
        int gasdfadfafwjlefkjskdjfsdf; // 이렇게는 사용하지 말자

        /*
        * 카멜케이스
        *
        * 합성어일 경우 첫단어는 모두 소문자, 두번째 시작 단어는 대문자로 시작한다.
        * 자바에서는 클레스명만 유일하게 대문자로 시작한다.
         */
        int maxAge = 20;
        int minAge = 10;
